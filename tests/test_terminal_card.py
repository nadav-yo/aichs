...
def test_terminal_card_copy_text_is_plain_output(qapp):
    ...
    card.finish(0, detail="exit 0", ref="#term[card123:1:2]")

    assert card.copy_text() == "alpha\nbeta"
    assert card.copy_ref() == "#term[card123:1:2]"
...
def test_terminal_output_copy_uses_selection_and_precise_hidden_reference(qapp):
    ...
    assert bytes(mime.data(TERMINAL_REF_MIME)).decode("utf-8") == "#term[card123:2:2]"


def test_terminal_output_partial_line_copy_uses_containing_line_reference(qapp):
    card = TerminalCard()
    card.set_output("alpha\nbeta")
    card.finish(0, detail="exit 0", ref="#term[card123:1:2]")
    output = card._output
...
def test_terminal_output_preserves_terminal_id_in_selected_reference(qapp):
    card = TerminalCard()
    card.set_output("alpha\nbeta")
    card.finish(0, detail="exit 0", ref="#term[command42:1:2]")
    cursor = card._output.textCursor()
    cursor.setPosition(7)
    cursor.setPosition(10, cursor.MoveMode.KeepAnchor)
    card._output.setTextCursor(cursor)

    mime = card._output.copy_mime()

    assert bytes(mime.data(TERMINAL_REF_MIME)).decode("utf-8") == "#term[command42:2:2]"
...
def test_terminal_output_copy_without_selection_copies_plain_text_and_hidden_reference(qapp, monkeypatch):
    ...
    assert bytes(mime.data(TERMINAL_REF_MIME)).decode("utf-8") == "#term[card123:1:2]"
...
def test_terminal_output_drag_mime_is_reference_link(qapp):
    ...
    assert mime.text() == "#term[card123:2:2]"
    assert bytes(mime.data(TERMINAL_REF_MIME)).decode("utf-8") == "#term[card123:2:2]"


def test_terminal_output_partial_line_drag_uses_containing_line_reference(qapp):
    card = TerminalCard()
    card.set_output("alpha\nbeta")
    card.finish(0, detail="exit 0", ref="#term[card123:1:2]")
    output = card._output
...
def test_collapsed_terminal_card_copy_mime_is_reference_only(qapp):
    ...
    assert mime.text() == "#term[card123:1:2]"
    assert bytes(mime.data(TERMINAL_REF_MIME)).decode("utf-8") == "#term[card123:1:2]"
...
def test_collapsed_terminal_card_copy_puts_hidden_reference_on_clipboard(qapp, monkeypatch):
    ...
    mime = clipboard.mimeData()
    assert clipboard.text() == "#term[card123:1:2]"
    assert bytes(mime.data(TERMINAL_REF_MIME)).decode("utf-8") == "#term[card123:1:2]"
